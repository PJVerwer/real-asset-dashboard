...
st.set_page_config(page_title="Global Real Asset Universe", layout="wide")
...
with tab1:
    ...
    <div class="page-guide">
        ...
        <p>This project estimates the global stock of <b>real assets</b> — defined as structures (residential and non-residential buildings and infrastructure) and the land on which they sit. This tool provides a multi-level view of the built environment, anchoring on official OECD National Account data and extending estimates globally using Penn World Table bridges.</p>
    ...
    st.markdown(f"""
    <div class="oecd-card">
        <h2 style="margin-top:0;">Regional Subset: OECD Anchor Deep-Dive</h2>
        ...
        <div class="metric-row"><span class="label">Dwellings (Residential Buildings)</span><span class="value">${s['oecd_dwellings']}T</span></div>
        <div class="metric-row"><span class="label">Other Buildings & Structures (Commercial + Infrastructure)</span><span class="value">${s['oecd_other']}T</span></div>
        <div class="metric-row" style="background-color:#f8fafc; padding:10px; border-radius:8px;"><span class="label"><b>Total OECD Structures</b></span><span class="value"><b>${s['oecd_total_struc']}T</b></span></div>
        <div class="metric-row"><span class="label">Land (OECD Reported + US Imputed)</span><span class="value">${s['oecd_land']}T</span></div>
...
with tab2:
    ...
    if sort_choice == "Total Value": display_df = display_df.sort_values("Combined_bn", ascending=False)
    elif sort_choice == "Structures/GDP": display_df = display_df.sort_values("S_GDP", ascending=False)
    else: display_df = display_df.sort_values("Country")
            ...
            with cols[j]:
                ...
                <div class="country-card">
                    ...
                    <div style="background-color:#f1f5f9; padding:12px; border-radius:8px; margin-bottom:20px;">
                        <div class="metric-row"><span class="label">Structures</span><span class="value">${fmt(country['Structures_bn'])}bn</span></div>
                        <div class="metric-row"><span class="label">Land</span><span class="value">${fmt(country['Land_bn'])}bn</span></div>
                        <div class="metric-row" style="border:none; font-weight:800; font-size:1rem; padding-top:5px; color:#011627;">
                            <span class="label" style="color:#011627;">Combined Real Assets</span><span class="value">${fmt(country['Combined_bn'])}bn</span>
                        </div>
                    </div>
                    ...
                    <div class="metric-row"><span class="label">Real Assets (Struct) / GDP</span><span class="value">{fmt(country['S_GDP'], 'x')}</span></div>
                    <div class="metric-row"><span class="label">Combined Assets / GDP</span><span class="value">{fmt(country['C_GDP'], 'x')}</span></div>
                    <div class="metric-row"><span class="label">Structures / Stock Market Cap</span><span class="value">{fmt(country['S_MC'], 'x')}</span></div>
                    <div class="metric-row" style="border:none;"><span class="label">Structures / Fixed Assets</span><span class="value">{pct(country['S_FA'])}</span></div>
                </div>
                """, unsafe_allow_html=True)
